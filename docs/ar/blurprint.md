...
يوضّح هذا المستند كيفية عمل xcoder داخليًا: أهداف التصميم، التجريدات الأساسية، المحركات، نظام المهارات، خادم API والواجهة، طوبولوجيا النشر، مرجع الدليل، نقاط التوسعة، والفجوات المعروفة.
...
3. **CLI وقاعدة البيانات مفصولان.** تشغيل `xcoder --task "..."` لا يفتح اتصال قاعدة بيانات أبدًا. فقط خادم API (الذي يدعم الواجهة) يكتب إلى قاعدة البيانات.
...
شغّل `xcoder --skills` للاطلاع على القائمة الحية للمهارات مع الأدوار والمشغلات.
...
- مع `--remote <ip>`: يعمل SSH إلى المضيف البعيد (`REMOTE_SSH_USER`/`REMOTE_SSH_PASSWORD` من `.env`) وينشر هناك؛ و`--remote-path` الافتراضي هو `/opt/xcoder`.
