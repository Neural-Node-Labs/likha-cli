...
Как вызывать CLI likha, запускать задачи, управлять API-сервером и интерфейсом, выбирать движок оркестрации и запускать тесты.
...
Позиционный аргумент `[task]` эквивалентен `--task <description>`. После сборки CLI находится в `dist/cli/index.js`; npm-скрипты и глобальный бинарник `likha` оба указывают на него.
...
likha "Отрефактори модуль аутентификации для использования JWT-токенов"

# Explicit task option with the lean engine
likha --engine lean --task "Проанализируй покрытие тестами"
...
likha --lesson "Всегда проверяй пути к файлам перед записью"

# Fully autonomous mode — auto-answers ALL interactive prompts
likha --auto --task "Настрой конвейер CI/CD"
...
```bash
# Force Plan Mode on
likha --plan --task "Сложная задача"

# Force Plan Mode off
likha --no-plan --task "Быстрая задача"

# Run as a single ReAct loop (disable phase planning)
likha --single-phase --task "Сложная задача"
```
...
likha поставляется с четырьмя взаимозаменяемыми движками оркестрации, все реализуют интерфейсы `IReactEngine` / `IReactEngineV2`. Выберите один с помощью `--engine <name>`:

```bash
likha --engine <name> --task "Перечисли все файлы TypeScript в src/"
```
