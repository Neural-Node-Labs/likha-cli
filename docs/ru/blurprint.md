...
Как xcoder работает внутри: цели проектирования, основные абстракции, движки, система навыков, API-сервер и интерфейс, топология развёртывания, справочник по каталогам и точки расширения.
...
3. **CLI и база данных развязаны.** Запуск `xcoder --task "..."` никогда не открывает соединение с базой данных. Только API-сервер (который обслуживает интерфейс) сохраняет данные в базу.
...
Запустите `xcoder --skills`, чтобы увидеть живой список навыков с ролями и триггерами.
...
- **С `--remote <ip>`:** подключается по SSH к удалённому хосту (`REMOTE_SSH_USER`/`REMOTE_SSH_PASSWORD` из `.env`) и развёртывает там вместо этого; `--remote-path` по умолчанию — `/opt/xcoder`.
